...
export default function Dashboard() {
  ...
  // ── Notifications ─────────────────────────────────────────────────────────
  useEffect(() => {
    fetch(`${BASE_URL}/notifications?limit=10&unread_only=false`, {
      headers: authHeaders,
    })
      .then((r) => (r.ok ? r.json() : []))
  ...
  //   try {
  //     const res = await fetch(`${BASE_URL}/run-inference`, {
  //       method: "POST",
  //       headers: authHeaders,
  //     });
  //     if (res.ok)
  //       alert("✅ Inference triggered! New predictions will appear shortly.");
  ...
  const statCards = [
    {
      ...
      sub: accuracy
        ? `${accuracy.correct_predictions}/${accuracy.total_predictions_analyzed} correct`
        : "Loading…",
    ...
    },
    {
      ...
      sub: accuracy
        ? `${accuracy.recent_predictions_count} recent predictions`
        : "Loading…",
    ...
    },
    {
      ...
      sub: accuracy
        ? `${accuracy.high_confidence_predictions} predictions`
        : "Loading…",
    ...
    },
    {
      ...
      value: accuracy
        ? accuracy.total_predictions_analyzed.toLocaleString()
        : "—",
      sub: accuracy
        ? `Updated ${new Date(accuracy.last_updated).toLocaleDateString()}`
        : "Loading…",
      ...
      <aside className="hidden lg:flex flex-col w-60 bg-slate-900/60 border-r border-slate-800 p-5 shrink-0">
        ...
        >
          ...
          <div className="truncate">
            ...
            <span
              className={`text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded ${user?.is_admin ? "text-purple-300 bg-purple-500/20" : "text-blue-300 bg-blue-500/20"}`}
            >
        ...
        <header className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
          <div>
            ...
            <p className="text-slate-600 text-xs mt-0.5">
              Real-time EPL analytics & AI predictions
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {statCards.map((s) => (
                  ...
                  >
                    ...
                    <p className="text-[10px] mt-1.5 text-slate-500 font-medium">
                      {s.sub}
                    </p>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  ...
                  >
                    {a.label === "Run Inference" && runningInference ? (
                ...
                {predictions.length === 0 ? (
                  ...
                  <div className="grid lg:grid-cols-2 gap-3">
                    {predictions.map((p) => (
                      <div
                        key={p.id}
                        className="flex items-center justify-between p-3 bg-slate-950/60 border border-slate-800 rounded-lg hover:border-slate-700 transition-all cursor-pointer"
                      >
                        <div className="flex justify-between w-[40%]">
                          <div className="flex flex-col items-center justify-center">
                            <img
                              src={`${BASE_URL}${p.home_team_logo}`}
                              className="h-6"
                            />
                        ...
                        <div className="text-right">
                          <p
                            className={`text-xs font-bold ${outcomeColor(p.outcome)}`}
                          >
                            {outcomeLabel(p.outcome)}
                          </p>
                          {p.confidence != null && (
                            <p className="text-[10px] text-slate-600 mt-0.5">
                              {confPct(p.confidence)}% confidence
                            </p>
                          )}
                        </div>
                      </div>
          ...
          {activeNav === "Predictions" && (
            <div className="space-y-4">
              <p className="text-slate-500 text-sm">
                {allPredictions.length} upcoming predictions
              </p>
                  ...
                  {allPredictions.map((p) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[10px] text-slate-600 font-bold uppercase px-2">
                          vs
                        </span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${
                              outcomeColor(p.outcome) === "text-blue-400"
                                ? "border-blue-500/40 bg-blue-500/10 text-blue-400"
                                : outcomeColor(p.outcome) === "text-yellow-400"
  ...
  <div className="mt-4 space-y-4">
    ...
    {/* DEFENCE */}
    {p.h_defending != null && p.a_defending != null && (
      <div>
        <div className="flex justify-between text-[11px] mb-1">
          <span className="font-bold text-blue-400">
            {p.h_defending.toFixed(2)}
          </span>
    ...
    {/* EFFICIENCY */}
    {p.h_efficiency != null && p.a_efficiency != null && (
      <div>
        <div className="flex justify-between text-[11px] mb-1">
          <span className="font-bold text-blue-400">
            {p.h_efficiency.toFixed(2)}
          </span>
  ...
  const load = async (newSkip = 0) => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/historical-matches?skip=0&limit=20`,
        { headers: authHeaders },
      );
...
const resultColor = (ftr) => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-slate-500 text-sm">
        {items.length} predictions loaded
      </p>
  ...
  <div
    key={p.id}
    className="p-4 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-all space-y-4"
  >
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="text-center">
        ...
        <p className={`text-[10px] font-bold ${resultColor(p.ftr)}`}>
          {resultLabel(p.ftr)}
        </p>
      </div>
