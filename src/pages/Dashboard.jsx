...
import {
  ...
  X, CheckCircle, XCircle, Activity,
} from "lucide-react";
import {
...
const navItems = [
  ...
  { name: "Prediction History",     icon: <History         size={18} /> },
  { name: "Evaluation",  icon: <BarChart2       size={18} /> },
];
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="p-2 rounded-lg bg-slate-800 border border-slate-700 hover:border-red-500/50 hover:bg-red-500/5 transition-all"
              title="Logout"
            >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          ...
          {activeNav === "Evaluation" && (
            <EvaluationTab accuracy={accuracy} loadingStats={loadingStats} authHeaders={authHeaders} />
          )}
        </div>
      </main>
    </div>
...
// ─── Evaluation Tab ───────────────────────────────────────────────────────────
function EvaluationTab({ accuracy, loadingStats, authHeaders }) {
  if (loadingStats) {
    return (
  ...
  if (!accuracy) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-2">
        ...
        <p className="text-slate-400">Unable to load accuracy metrics</p>
      </div>
    );
  }

  const KPICard = ({ label, value, subtitle, icon: Icon, color }) => (
    <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-5 hover:border-slate-600 transition-all">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-2">{label}</p>
  ...
  const accuracyData = [
    { outcome: "Home Win", accuracy: accuracy.home_win_accuracy, fill: "#3b82f6" },
    { outcome: "Draw", accuracy: accuracy.draw_accuracy, fill: "#fbbf24" },
    { outcome: "Away Win", accuracy: accuracy.away_win_accuracy, fill: "#a855f7" },
  ];

  const radarData = [
    { outcome: "Home Win", accuracy: accuracy.home_win_accuracy },
    { outcome: "Draw", accuracy: accuracy.draw_accuracy },
    { outcome: "Away Win", accuracy: accuracy.away_win_accuracy },
  ];

  const trend = accuracy.recent_accuracy_30_days >= accuracy.overall_accuracy ? "up" : "down";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          value={`${accuracy.overall_accuracy.toFixed(1)}%`}
          subtitle={`${accuracy.correct_predictions}/${accuracy.total_predictions_analyzed} correct`}
          icon={Zap}
          color="text-yellow-400"
        />
        <KPICard
          label="Total Predictions"
          value={accuracy.total_predictions_analyzed}
          subtitle="matches analyzed"
          icon={Activity}
          color="text-blue-400"
        />
        <KPICard
          label="Correct Predictions"
          value={accuracy.correct_predictions}
          subtitle={`${((accuracy.correct_predictions / accuracy.total_predictions_analyzed) * 100).toFixed(1)}% success`}
          icon={CheckCircle}
          color="text-emerald-400"
        />
        <KPICard
          label="30-Day Accuracy"
          value={`${accuracy.recent_accuracy_30_days.toFixed(1)}%`}
          subtitle={`${accuracy.recent_predictions_count} recent`}
          icon={Shield}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
            ...
            <BarChart data={accuracyData}>
              ...
              <Bar dataKey="accuracy" radius={[8, 8, 0, 0]}>
                {accuracyData.map((entry, index) => (
                  <Bar key={`bar-${index}`} dataKey="accuracy" fill={entry.fill} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        {/* Expertise Radar */}
        <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-6">
          <h3 className="text-white font-bold text-lg mb-4">Expertise Radar</h3>
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData}>
              <PolarGrid stroke="#334155" />
              <PolarAngleAxis dataKey="outcome" stroke="#94a3b8" tick={{ fontSize: 12 }} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Reliability Status */}
        <div className={`border rounded-lg p-6 ${accuracy.high_confidence_predictions > 0 ? "bg-emerald-500/10 border-emerald-500/30" : "bg-yellow-500/10 border-yellow-500/30"}`}>
          <div className="flex items-start justify-between mb-4">
            <h3 className="text-white font-bold text-lg">Reliability Status</h3>
            <span className={`text-xs font-bold uppercase px-2 py-1 rounded ${accuracy.high_confidence_predictions > 0 ? "bg-emerald-500/20 text-emerald-400" : "bg-yellow-500/20 text-yellow-400"}`}>
              {accuracy.high_confidence_predictions > 0 ? "Trustworthy" : "Threshold Not Met"}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-2">High Confidence</p>
              <p className="text-2xl font-black text-emerald-400">{accuracy.high_confidence_predictions || "—"}</p>
            </div>
            <div>
              <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-2">Accuracy HConf</p>
              <p className="text-2xl font-black text-blue-400">{accuracy.high_confidence_accuracy > 0 ? `${accuracy.high_confidence_accuracy.toFixed(1)}%` : "—"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-white font-bold text-lg">Momentum</h3>
            <span className={`text-xs font-bold ${trendColor}`}>{trend === "up" ? "↑ Improving" : "↓ Declining"}</span>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-400">Overall</span>
              <span className="text-xl font-black text-yellow-400">{accuracy.overall_accuracy.toFixed(1)}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-400">30-Day</span>
              <span className="text-xl font-black text-purple-400">{accuracy.recent_accuracy_30_days.toFixed(1)}%</span>
            </div>
            <div className="text-xs text-slate-500 mt-4">
              {trend === "up"
                ? "✓ Model performing well over recent period"
                : "⚠ Model performance declining - possible drift"}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-slate-400 text-xs uppercase tracking-widest font-bold">Last Updated</p>
            <p className="text-sm font-bold text-blue-400">{new Date(accuracy.last_updated).toLocaleString()}</p>
          </div>
        </div>
      </div>
    </div>
...
function HistoryTab({ authHeaders, formatDate }) {
  ...
  const load = async (newSkip = 0) => {
    ...
    try {
      ...
      const arr  = Array.isArray(data) ? data : (data?.data ?? data?.items ?? []);
      console.log("[HistoryTab] Parsed array length:", arr.length);
      if (newSkip === 0) setItems(arr);
      else setItems((prev) => [...prev, ...arr]);
      setHasMore(arr.length === LIMIT);
      setSkip(newSkip + LIMIT);
    } catch (e) {
      console.error("Failed to fetch historical matches:", e);
    } finally { setLoading(false); }
  };

  useEffect(() => { 
    console.log("[HistoryTab] Component mounted, calling load(0)");
    load(0); 
  }, [authHeaders]);
